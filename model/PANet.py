...
class PANet(nn.Module):
    def __init__(self, load_weights=False):
        ...
        #self.conv1 = DeformConv2D(512, 512)
        #self.bn1 = nn.BatchNorm2d(512)
        #self.conv2 = DeformConv2D(512, 512)
        #self.bn2 = nn.BatchNorm2d(512)
        #self.conv3 = DeformConv2D(512, 512)
        #self.bn3 = nn.BatchNorm2d(512)
        self.conv4 = DeformConv2D(512, 256)
        self.bn4 = nn.BatchNorm2d(256)
        self.conv5 = DeformConv2D(256, 128)
        self.bn5 = nn.BatchNorm2d(128)
        self.conv6 = DeformConv2D(128, 64)
        ...
        if not load_weights:
            ...
            for i in range(len(self.frontend.state_dict().items())):
                ...
                fsd[temp_key] = list(mod.state_dict().items())[i][1]
            self.frontend.load_state_dict(fsd)
    def forward(self, x, dm):
        x = self.frontend(x)
        x = self.backend(x)
        #x = self.relu(self.bn1(self.conv1(x, dm)))
        #x = self.relu(self.bn2(self.conv2(x, dm)))
        #x = self.relu(self.bn3(self.conv3(x, dm)))
        x = self.relu(self.bn4(self.conv4(x, dm)))
        x = self.relu(self.bn5(self.conv5(x, dm)))
        x = self.relu(self.bn6(self.conv6(x, dm)))
        x = self.output_layer(x)
        return x
